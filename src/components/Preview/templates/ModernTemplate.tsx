...
export function ModernTemplate() {
  const { personal, summary, experience, education, skills, projects, sections, settings } =
    useResumeStore();
  ...
  const contactItems = [
    { label: t.personal.email,    value: personal.email,    href: personal.email    ? `mailto:${personal.email}`    : undefined },
    { label: t.personal.phone,    value: personal.phone,    href: personal.phone    ? `tel:${personal.phone}`       : undefined },
    { label: t.personal.location, value: personal.location, href: undefined },
    { label: t.personal.website,  value: personal.website,  href: personal.website  ? (personal.website.startsWith('http')  ? personal.website  : `https://${personal.website}`)  : undefined },
    { label: t.personal.linkedin, value: personal.linkedin, href: personal.linkedin ? (personal.linkedin.startsWith('http') ? personal.linkedin : `https://${personal.linkedin}`) : undefined },
    { label: t.personal.github,   value: personal.github,   href: personal.github   ? (personal.github.startsWith('http')   ? personal.github   : `https://${personal.github}`)   : undefined },
  ].filter(({ value }) => value);
  ...
  return (
    ...
    >
      {/* ── Left sidebar ──────────────────────────────────── */}
      <aside
        className="w-[200px] shrink-0 flex flex-col gap-5 px-5 pt-8 pb-8"
        style={{
            ...
            <div className="flex flex-col gap-1.5">
              {contactItems.map(({ label, value, href }) => (
                <div key={label}>
                  ...
                  {href
                    ? <a href={href} className="text-[9.5px] text-gray-700 break-all leading-tight">{value}</a>
                    : <p className="text-[9.5px] text-gray-700 break-all leading-tight">{value}</p>
                  }
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-3">
              {education.map((edu) => (
                <div key={edu.id}>
                  ...
                  <p className="text-[9px] text-gray-600 mt-0.5">{edu.institution}</p>
                  ...
                  {edu.location && (
                    <p className="text-[9px] text-gray-400">{edu.location}</p>
                  )}
                  {edu.gpa && (
                    <p className="text-[9px] text-gray-400">GPA: {edu.gpa}</p>
                  )}
                </div>
              ))}
            </div>
            ...
            {experience.map((exp) => (
              <div key={exp.id} className="mb-3 last:mb-0">
                <div className="flex justify-between items-baseline gap-2 flex-wrap">
                  <div>
                    <span className="text-[12px] font-semibold text-gray-900">{exp.position}</span>
                    {exp.company && (
                      <span className="text-[11px] text-gray-500 ml-1.5">· {exp.company}</span>
                    )}
                  </div>
                ...
                {exp.location && (
                  <p className="text-[9.5px] text-gray-400 mt-0.5">{exp.location}</p>
                )}
                {exp.description && (
                  <p className="text-[10.5px] text-gray-600 leading-relaxed mt-0.5" style={{ whiteSpace: 'pre-line' }}>
                    {exp.description}
                  </p>
                )}
                {exp.highlights.filter(Boolean).length > 0 && (
                  <ul className="mt-1 space-y-0.5 pl-3">
                    {exp.highlights.filter(Boolean).map((h, i) => (
                      <li key={i} className="text-[11px] text-gray-700 leading-relaxed list-disc">
                        {h}
                      </li>
                    ))}
                  </ul>
