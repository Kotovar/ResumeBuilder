...
import type { AccentColor, FontFamily, Lang, ResumeSettings, TemplateType } from '../../types/resume';
...
function TemplatePicker({ t, settings, updateSettings }: PickerProps) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs font-medium text-gray-400">{t.toolbar.template}</span>
      <div className="flex rounded-md overflow-hidden border border-gray-200">
        {(['classic', 'modern'] as TemplateType[]).map((tmpl) => (
          <button
            ...
            style={settings.template === tmpl ? { backgroundColor: 'var(--accent)' } : {}}
          >
            {tmpl === 'classic' ? t.toolbar.classic : t.toolbar.modern}
          </button>
        ))}
      </div>
    </div>
...
function FontPicker({ t, settings, updateSettings }: PickerProps) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs font-medium text-gray-400">{t.toolbar.font}</span>
      <select
...
function AccentPicker({ t, settings, updateSettings }: PickerProps) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs font-medium text-gray-400">{t.toolbar.color}</span>
...
function LangPicker({ t, settings, updateSettings }: PickerProps) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs font-medium text-gray-400">{t.toolbar.language}</span>
      <div className="flex rounded-md overflow-hidden border border-gray-200">
        {(['en', 'ru'] as Lang[]).map((lng) => (
          <button
            ...
            style={settings.lang === lng ? { backgroundColor: 'var(--accent)' } : {}}
          >
            {lng}
          </button>
        ))}
...
// ── Main Toolbar ───────────────────────────────────────────
export function Toolbar({ onExportPDF, onExportJSON, onImportJSON, onReset }: Props) {
        ...
        <>
          ...
          <div className="md:hidden absolute top-full left-0 right-0 z-20
            ...
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => { onImportJSON(); setMenuOpen(false); }}
