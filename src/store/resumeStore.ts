      ...
      migrate: (raw, fromVersion) => {
        ...
        if (fromVersion < 2) {
          ...
          state.experience = (state.experience ?? []).map((e) => ({
            ...e,
            description: (e as ExperienceEntry & { description?: string }).description ?? '',
          }));
          // Add lang to settings
          state.settings = {
            ...state.settings,
            lang: (state.settings as ResumeSettings & { lang?: string }).lang as ResumeSettings['lang'] ?? 'en',
            // Migrate removed font 'raleway' to 'ptsans'
            fontFamily: state.settings.fontFamily === ('raleway' as string)
              ? 'ptsans'
              : (state.settings.fontFamily as ResumeSettings['fontFamily']) ?? 'inter',
          };
        }
        return state;
      },
